...
#include "reg_defines/7050.h"	//required for SCI stuff
//#include "npk_ver.h"
#include "platf.h"
...
#define SIDFL_WB_DLEN32	32	//bytes per block
...
static void sci_rxidle(unsigned ms) {
	...
	while (1) {
		...
		if (SCI2.SSR.BYTE & 0x78) {
			/* RDRF | ORER | FER | PER :reset timer */
			t0 = get_mclk_ts();
			SCI2.SSR.BYTE &= 0x87;	//clear RDRF + error flags
		}
	}
}
...
static void sci_txblock(const uint8_t *buf, uint32_t len) {
	for (; len > 0; len--) {
		while (!SCI2.SSR.BIT.TDRE) {}	//wait for empty
		SCI2.TDR = *buf;
		buf++;
		SCI2.SSR.BIT.TDRE = 0;		//start tx
	}
}
 ...
 * updates SCI2 settings : 62500 bps
...
void cmd_init(u8 brrdiv) {
	cmstate = CM_IDLE;
	flashstate = FL_IDLE;
	SCI2.SCR.BYTE &= 0xCF;	//disable TX + RX
	SCI2.BRR = brrdiv;		// speed = (div + 1) * 625k
	SCI2.SSR.BYTE &= 0x87;	//clear RDRF + error flags
	SCI2.SCR.BYTE |= 0x30;	//enable TX+RX , no RX interrupts for now
	return;
}
...
static void cmd_flash_utils(struct iso14230_msg *msg) {
	...
	case SIDFL_WB:
		...
		for (pos = 5; pos < 133; pos += 32) {
			
			rv = platf_flash_wb(tmp, (u32) &msg->data[pos], SIDFL_WB_DLEN32);
			if (rv) {
				rv = (rv & 0xFF) | 0x80;	//make sure it's a valid extented NRC
				goto exit_bad;
			}
